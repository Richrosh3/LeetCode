...
Explanation: 
    Recursive solution. 
    If the node is None, we can return a value of 0. 
    Otherwise, we can return 1 + countNodes(root.left) + countNodes(root.right). This allows us to recursively receive 
    the number of nodes in each subtree. 
    
    Time complexity is O(n) because we traverse every node. 
    Space complexity is O(1) as no new space was created. 
"""